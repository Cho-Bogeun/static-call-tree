...
기본 의존성인 PyPI `libclang` 휠은 네이티브 라이브러리를 자체 포함한다. 시스템에
설치된 clang 을 쓰려면:
...
파이썬 바인딩과 네이티브 라이브러리의 메이저 버전이 다르면 `undefined symbol` 로
죽는다. 둘의 버전을 맞춰야 한다.
...
방향이 문맥대로 나오는지, 출력이 스키마를 만족하는지를 본다. libclang 을 못 부르는
환경에서는 파싱이 필요한 테스트만 건너뛴다.
